...
def test_az_under_10k(record_xml_attribute):
    ...
    record_xml_attribute(
        'name', 'Land in Arizona with > 5 acres with price < 10k')
    fail = None
    ...
    web_app_setup.go_to_landwatch_search(driver, SiteToScrape.landwatch, 'az', 5, max_price=6000)
    ...
    try:
        ...
        screenshots.take_screenshot(driver, 'AZland')
        print('We found {} optimal listings to consider {}:'.format(len(optimal_results_list), optimal_results_list))
        search_page.write_to_csv('test-reports/AZ_GT5ACRES_LT6k.csv', optimal_results_list)
