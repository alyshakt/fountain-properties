...
def test_ut_under_10k(record_xml_attribute):
    ...
    record_xml_attribute(
        'name', 'Land in Utah with > 5 acres with price < 10k')
    fail = None
    ...
    web_app_setup.go_to_landwatch_search(driver, SiteToScrape.landwatch, 'ut', 5, max_price=6000)
    ...
    try:
        ...
        screenshots.take_screenshot(driver, 'UTland')
        print('We found {} optimal listings to consider {}:'.format(len(optimal_results_list), optimal_results_list))
        search_page.write_to_csv('test-reports/UT_GT5ACRES_LT6k.csv', optimal_results_list)
